...
<body>
    ...
    echo "<table>";
    echo "<tr><th>Event</th><th>Event Theme</th><th>Date & Time</th><th>Event Location</th><th class='action-cell'>Action</th></tr>";

    foreach ($events as $event) {
        echo "<tr>";
        echo "<td class='{$event[0]}'>{$event[0]}</td>";
        echo "<td>{$event[1]}</td>";
        echo "<td>{$event[2]}</td>";
        echo "<td>{$event[3]}</td>";
        echo "<td class='action-cell'><a class='signup-btn' href='signedup.php?event={$event[0]}'>View Signed Up</a></td>";
        echo "</tr>";
    }
